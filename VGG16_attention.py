...
from tensorflow.keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Dense, Flatten, Reshape, Multiply, Concatenate
...
# 获取VGG16的输出，并应用CBAM注意力模块
vgg_output = vgg_base.output
attention_output = cbam_block(vgg_output)

# 将输出展开
vgg_flatten = Flatten()(attention_output)

# 添加自定义的全连接层
x = Dense(256, activation='relu')(vgg_flatten)
x = Dense(128, activation='relu')(x)
output_layer = Dense(train_augmented.num_classes, activation='softmax')(x)

# 创建最终模型
custom_model = Model(inputs=vgg_base.input, outputs=output_layer)
...
model_checkpoint = ModelCheckpoint(filepath='/public/home/shenyin_wsb_2606/Art_class/best_vgg16_cbam.h5', monitor='val_loss', save_best_only=True)
...
plt.ylabel('VGG16-CBAM-Accuracy')
plt.xlabel('Epoch')
...
plt.title('VGG16-CBAM-Model Loss')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
...
plt.title('VGG16-CBAM-Validation AUC')
plt.ylabel('AUC')
plt.xlabel('Epoch')

plt.tight_layout()
plt.savefig('/public/home/shenyin_wsb_2606/Art_class/vgg16_cbam.png')
plt.show()
